            ...
            {order.items.map((item, index) => (
              <div key={index} className={`relative bg-white text-gray-500/70 ${order.items.length !== index+1 && "border-b" } border-gray-300 flex flex-col md:flex-row md:items-center justify-between p-4 py-5 md:gap-16 w-full max-w-4xl `}>
                <div className='flex items-center mb-4 md:mb-0'>
                  <div className='bg-primary/10 p-4 rounded-lg'>
                    {item.product ? (
                      ...
                      <span className="text-gray-400">Product deleted</span>
                    )}
                  </div>
                  <div className='ml-4'>
                    <h2 className='text-4xl font-medium text-gray-800'>{item.product ? item.product.name : "Deleted Product"}</h2>
                ...
                <p className='text-primary text-lg font-medium'>
                    Amount: {currency}{item.product ? item.product.offerPrice * item.quantity : "0.00"}
                </p>
              </div>
            ))}
          </div>
